package com.bignerdranch.nyethack

import kotlin.random.Random

interface Fightable {
    val name: String
    val healthPoints: Int
    val diceCount: Int
    val diceSides: Int

    fun takeDamage(damage: Int)

    fun attack(opponent: Fightable) {
        val damageRoll = (0..<diceCount).sumOf {
            Random.nextInt(diceSides + 1)
        }
        narrate("$name deals $damageRoll to ${opponent.name}")
        opponent.takeDamage(damageRoll)
    }
}

abstract class Monster(
    override val name: String,
    val description: String,
    override var healthPoints: Int
) : Fightable {

    override fun takeDamage(damage: Int) {
        healthPoints -= damage
    }
}

class Goblin(
    name: String = "Goblin",
    description: String = "A nasty-looking goblin",
    healthPoints: Int = 30
) : Monster(name, description, healthPoints) {

    override val diceCount = 3
    override val diceSides = 4
}

class Draugr(
    name: String = "Draugr",
    description: String = "A nasty-looking draugr",
    healthPoints: Int = 40
) : Monster(name, description, healthPoints) {

    override val diceCount = 2
    override val diceSides = 8
}

class Werewolf(
    name: String = "Werewolf",
    description: String = "A vicious werewolf",
    healthPoints: Int = 80
) : Monster(name, description, healthPoints) {

    override val diceCount = 3
    override val diceSides = 20
}

class Dragon(
    name: String = "Dragon",
    description: String = "A mighty dragon",
    healthPoints: Int = 200
) : Monster(name, description, healthPoints){

    override val diceCount = 1
    override val diceSides = 50
}